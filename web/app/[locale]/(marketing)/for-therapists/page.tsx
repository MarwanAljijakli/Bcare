...
export async function generateMetadata({ params }: { params: Promise<{ locale: AppLocale }> }) {
  ...
  const t = await getTranslations({ locale, namespace: 'marketing.forTherapists.hero' });
  return pageMetadata({ locale, path: 'for-therapists', title: t('title') });
}

const ICONS = [Users, ListChecks, RotateCcw, Languages, EyeOff, ShieldCheck] as const;
...
function Hero() {
  const t = useTranslations('marketing.forTherapists.hero');
  return (
...
function Benefits() {
  const t = useTranslations('marketing.forTherapists.benefits');
  return (
    <Section tone="muted">
